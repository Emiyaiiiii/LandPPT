    ...
    if token:
        try:
            ...
            if new_user:
                ...
                # Method 3: Check Sec-Fetch-Site header (if available)
                ...
                logger.info(f"iframe detection: referer={referer[:50] if referer else ''}, referer_iframe={referer_indicates_iframe}, "
                          f"param_iframe={param_indicates_iframe}, fetch_site={fetch_site}, "
                          f"fetch_iframe={fetch_indicates_iframe}, final={is_iframe}")
                ...
                if app_config.enable_iframe_support and is_iframe:
                    ...
                    logger.info(f"iframe mode: samesite=none, secure={cookie_secure}")
                else:
                    # Standard mode
                    cookie_samesite = "lax"
                    cookie_secure = False
                    logger.info(f"standard mode: samesite=lax, secure=False (enable_iframe={app_config.enable_iframe_support}, is_iframe={is_iframe})")
                
                # For iframe cross-domain: redirect with session_id in URL
                # This is a workaround when cookies are blocked by browser
                if is_iframe and app_config.enable_iframe_support:
                    # Redirect to dashboard with session_id in URL
                    redirect_url = f"/dashboard?_session_id={new_session_id}"
                    response = RedirectResponse(url=redirect_url, status_code=302)
                    logger.info(f"iframe redirect with session in URL: {redirect_url}")
                else:
                    # Standard redirect
                    response = RedirectResponse(url="/dashboard", status_code=302)
                
                # Always try to set cookie (for non-iframe or same-domain iframe)
                response.set_cookie(
                    key="session_id",
                ...
                logger.info(f"Root cookie set: samesite={cookie_samesite}, secure={cookie_secure}")
